    ...
    <div className="bg-gradient-to-tr from-[#eef2ff] via-[#faf8ff] to-[#f5f3ff] font-body-md text-[#131b2e] min-h-screen relative overflow-hidden flex items-center justify-center w-full">
        ...
        <div className="glass-panel w-full max-w-[480px] p-8 md:p-10 rounded-2xl border border-white/60 bg-white/75 backdrop-blur-md shadow-[0_15px_35px_-5px_rgba(0,40,150,0.08)] hover:shadow-[0_25px_50px_-10px_rgba(0,40,150,0.15)] transition-all duration-500 ease-in-out hover:-translate-y-1">
          ...
          <div className="mt-8 text-center space-y-4 select-none">
            ...
            <div className="flex items-center justify-center gap-6 pt-6 border-t border-[#c3c5d9]/40">
              <a className="text-xs text-[#737688] hover:text-[#434656] transition-colors" href="#" onClick={(e) => { e.preventDefault(); openInfo("Privacy Policy", "This Retail Management Suite takes your data security seriously:\n\n• User Logs: We perform real-time activity tracking (logged by IP) for security audits.\n• Data Control: All sales, transactions, invoices, and product logs are stored securely in a dedicated database cluster.\n• Session Protection: In-memory session management is used. Authentication states are fully deleted upon tab close or reload.\n• Integrity: Local thermal receipt downloads are generated directly via secure public share tokens to prevent IDOR attacks."); }}>Privacy Policy</a>
      ...
      {infoModal.show && (
        <div className="fixed inset-0 z-50 bg-[#131b2e]/40 backdrop-blur-sm flex items-center justify-center p-6 animate-fade-in select-none">
          <div className="bg-white rounded-2xl max-w-sm w-full p-6 shadow-2xl border border-[#c3c5d9]/40 space-y-4">
